...
- автоматическое исправление всех документов без отдельной команды пользователя;
- создание второго каталога тестов, второго PRD или второго snapshot;
- превращение target-state материалов в as-built устав;
- создание отдельного result-файла на каждый прогон.
...
1. Общим оглавлением проекта считается только корневой `README.md`; ссылки на `docs/README.md` трактуются как stale path по `C11`.
