...
**Этот файл не заменяет PRD.** Он фиксирует: список тестов, ожидания, критерии приёмки, критерии проверки, команды, тонкости и известные ошибки.
...
**Команды:** секция [Test Commands](#test-commands) (`mvn -pl demo-tests -am test`, Allure report, sqlite inspect).
...
Команды, использованные в диалоге проверки среды, прогона тестов, разбора Event Handling и Allure. Репозиторий: `C:\Users\V\pod-logger-junit-demo`.

Рабочий Maven в сессии: `C:\Users\V\apache-maven-3.9.16` (PATH).  
Путь `C:\Program Files\apache-maven-3.9.16-bin.zip` — zip, не `bin`.  
`JAVA_HOME=C:\Program Files\Java\jdk-21`.

CTL в этом проекте — **Docker CLI** (локальный K3s через Testcontainers). `kubectl`/`oc` в прогоне не вызывались.
...
### Allure: results, list generated report, открытие
...
Полный список Shell-вызовов этой сессии (40), в порядке выполнения:
